...
void write_to_stdout(int data) { client_write_byte(data | 0x80); }
...
int uart_master(volatile unsigned int *sseg, volatile unsigned int *leds,
...
#pragma HLS INTERFACE m_axi depth = IOSIZE port = iomap offset =               \
    direct bundle = mem
