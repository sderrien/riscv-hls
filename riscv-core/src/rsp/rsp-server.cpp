...
mp_hash_lookup(enum mp_type type, unsigned long int addr) {
	int hv = addr % MP_HASH_SIZE;
	struct mp_entry *curr;
...
/*---------------------------------------------------------------------------*/
static struct mp_entry*
mp_hash_delete(enum mp_type type, unsigned long int addr) {
	int hv = addr % MP_HASH_SIZE;
...
static void rsp_read_all_regs() {
	...
	for (r = 0; r < MAX_GPRS; r++) {
		...
		//fprintf(stderr, "x[%d]=%08X\n", r, reg);

		reg2hex(read_reg((uint16_t) r), &(buf.data[r * 8]));
		//fprintf(stderr, "Sending x[%d]=%s\n", r, &(buf.data[r * 8]));
	}
	// Get the adjusted PC
	reg2hex(get_pc(), &(buf.data[32 * 8]));
	fprintf(stderr, "Sending pc=%s\n", &(buf.data[r * 8]));
...
static void rsp_read_mem(struct rsp_buf *buf) {
	...
	for (off = 0; off < len; off++) {
		...
		printf("buf[%02X]=%c\n",off*2,buf->data[off * 2]);
		printf("buf[%02X]=%c\n",off*2+1,buf->data[off * 2+1]);
	}
...
static void rsp_query(struct rsp_buf *buf) {
	...
	if (0 == strcmp("qAttached", buf->data)) {
		...
		put_str_packet("l");
			...
			== strncmp("qGetTLSAddr:", buf->data, strlen("qGetTLSAddr:"))) {
		...
		put_str_packet("OK");
			...
			== strncmp("qThreadExtraInfo,", buf->data,
					strlen("qThreadExtraInfo,"))) {
		/* Report that we are runnable, but the text must be hex ASCI
		 digits. For now do this by steam, reusing the original packet */
		sprintf(buf->data, "%02x%02x%02x%02x%02x%02x%02x%02x%02x", 'R', 'u',
				'n', 'n', 'a', 'b', 'l', 'e', 0);
		buf->len = strlen(buf->data);
		put_packet(buf);
...
static void rsp_vpkt(struct rsp_buf *buf) {
	if (0 == strncmp("vAttach;", buf->data, strlen("vAttach;"))) {
		...
		fprintf(stderr, "Unsupported command %s", buf->data);
		put_str_packet("S05");
		return;
		...
		return;
			...
			== strncmp("vFlashErase:", buf->data, strlen("vFlashErase:"))) {
		/* For now we don't support this. */
		fprintf(stderr, "Warning: RSP vFlashErase not supported: ignored\n");
		put_str_packet("E01");
		return;
	} else if (0
			== strncmp("vFlashWrite:", buf->data, strlen("vFlashWrite:"))) {
		/* For now we don't support this. */
