...
#define GDB_BUF_MAX 1024 + ((NUM_REGS)*8 + 1)
...
static void mp_hash_add(enum mp_type type, unsigned long int addr,
...
static void rsp_step_generic(unsigned long int addr, unsigned long int except);
...
void rsp_exception(unsigned long int except) {
  ...
  if ((0 != rsp.sigval) && (sigval != rsp.sigval)) {
    fprintf(stderr,
            "Warning: RSP signal %d received while signal "
...
static void rsp_get_client() {
  ...
  optval = 1;
  setsockopt(tmp_fd, SOL_SOCKET, SO_REUSEADDR, (char *)&optval, sizeof(optval));
...
static void rsp_client_request() {
  ...
  case 'g':
    // fprintf(stderr, "Warning: read all register command\n");
    rsp_read_all_regs();
    //		fprintf(stderr, "%s\n", buf->data);
    //		fprintf(stderr, "Done\n");
    return;
...
static void put_str_packet(const char *str) {
  ...
  if (len >= GDB_BUF_MAX) {
    fprintf(stderr,
            "Warning: String %s too large for RSP packet: "
            "truncated\n",
            str);
    len = GDB_BUF_MAX - 1;
  }
...
static void put_rsp_char(char c) {
  if (-1 == rsp.client_fd) {
    fprintf(stderr,
            "Warning: Attempt to write '%c' to unopened RSP "
            "client: Ignored\n",
            c);
    return;
  }
...
//   set_npc (addr);
//
//   /* Clear Debug Reason Register and watchpoint break generation in Debug
//   Mode
//      Register 2 */
...
static void rsp_read_mem(struct rsp_buf *buf) {
  ...
  if (2 != sscanf(buf->data, "m%x,%x:", &addr, &len)) {
    fprintf(stderr,
            "Warning: Failed to recognize RSP read memory "
            "command: %s\n",
            buf->data);
    put_str_packet("E01");
    return;
  ...
  buf->len = strlen(buf->data);
  // printf("Read memory request [%08X;%08X] \n",addr, len);
  // printf("Reply : %s\n",buf->data);
...
static void rsp_write_mem(struct rsp_buf *buf) {
  ...
  if (2 != sscanf(buf->data, "M%x,%x:", &addr, &len)) {
    fprintf(stderr,
            "Warning: Failed to recognize RSP write memory "
            "command: %s\n",
            buf->data);
    put_str_packet("E01");
    return;
  ...
  symdat = ((char *)memchr((const void *)(buf->data), ':', GDB_BUF_MAX)) + 1;
...
static void rsp_query(struct rsp_buf *buf) {
  if (0 == strcmp("qAttached", buf->data)) {
    ...
    /* Return info about active threads. We return just the constant
     OR1KSIM_TID to represent our single thread of control. */
    //	int nbMaxHart = cpu_info(MVENDOR_INFO);
    sprintf(buf->data, "m%x", 1); // cpu_info(MVENDOR_INFO));
    buf->len = strlen(buf->data);
    put_packet(buf);
    ...
    sprintf(buf->data, "%02x%02x%02x%02x%02x%02x%02x%02x%02x", 'R', 'u', 'n',
            'n', 'a', 'b', 'l', 'e', 0);
    buf->len = strlen(buf->data);
    put_packet(buf);
...
static void rsp_step(struct rsp_buf *buf) {
  ...
  if (0 == strcmp("s", buf->data)) {
    ...
    fprintf(stderr, "Warning: RSP step address %s not recognized: ignored\n",
            buf->data);
    addr = debug_get_pc(); /* Default uses current NPC */
  }
  if (verbose)
...
static void rsp_write_mem_bin(struct rsp_buf *buf) {
  ...
  bindat = ((char *)memchr((const void *)(buf->data), ':', GDB_BUF_MAX)) + 1;
  off = bindat - buf->data;
...
static void rsp_remove_matchpoint(struct rsp_buf *buf) {
  ...
  type = (enum mp_type)type_for_scanf;

  /* Sanity check that the length is 4 */
  if (type == BP_MEMORY && 4 != len) {
    fprintf(stderr,
  ...
  default:
    fprintf(stderr,
            "Warning: RSP matchpoint type %d not "
            "recognized: ignored\n",
            type);
    put_str_packet("E01");
    return;
...
static void rsp_insert_matchpoint(struct rsp_buf *buf) {
  ...
  case WP_WRITE:
    ...
    //{
    //   fprintf(stderr, "Watchpoints not supported to addresses other than
    //   WATCHPOINT_ADDR.\n"); put_str_packet ("");		/* Not supported
    //   */
    // }
    // else
...
void rsp_check_stall() {
  fprintf(stderr, "rsp_check_stall unsupported \n");
  put_str_packet("");
  return;
#ifdef TODO
  if (rsp.stepping) {
  ...
  else if (NULL != mp_hash_lookup(WP_WRITE, WATCHPOINT_ADDR) &&
           cyclesSinceReset >= resetAfterCycles) {
    rsp.stalled = 1;
    rsp.stepping = 0;
