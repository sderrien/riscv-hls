...
uint32_t debug_get_pc() { return cpu_getpc(); }
...
uint32_t debug_read_reg(uint16_t id) { return cpu_getreg(id); }

void debug_write_reg(uint16_t id, uint32_t value) { cpu_setreg(id, value); }

uint8_t debug_read_mem(uint32_t addr) { return cpu_memread_u8(addr); }
